...
Repo: https://github.com/burakbaslican/llmperf/releases/tag/v1.2.3  
(Private repo → `gh auth login` gerekli — public ise doğrudan indirilebilir.)

### Mac M2 Ultra — Docker Desktop (önerilen)

```bash
gh release download v1.2.3 -R burakbaslican/llmperf -p 'llmperf-docker-mac-arm64-1.2.3.tar.gz' \
  && tar -xzf llmperf-docker-mac-arm64-1.2.3.tar.gz \
  && cd llmperf-docker-mac-arm64-1.2.3 \
...
```bash
gh release download v1.2.3 -R burakbaslican/llmperf -p 'llmperf-mac-arm64-1.2.3.tar.gz' \
  && tar -xzf llmperf-mac-arm64-1.2.3.tar.gz \
  && cd llmperf-mac-arm64-1.2.3 \
  && ./install-mac.sh up
```
...
```bash
gh release download v1.2.3 -R burakbaslican/llmperf -p 'llmperf-1.2.3.tar.gz' \
  && tar -xzf llmperf-1.2.3.tar.gz \
  && cd llmperf-1.2.3 \
  && ./install.sh up
```
...
# → dist/llmperf-1.2.3.tar.gz                 (Linux / Docker)

./scripts/package-docker-mac.sh 1.2.3
# → dist/llmperf-docker-mac-arm64-1.2.3.tar.gz (Mac Docker Desktop)

./scripts/package-mac.sh 1.2.3
# → dist/llmperf-mac-arm64-1.2.3.tar.gz        (Mac native Python)
```
